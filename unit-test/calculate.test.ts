...
import { getEdge,  hasNode, IMakeNetworkResult, IRandomEdge, IRandomNode, maxWeight, minWeight, nodeWeightRange, randomNetwork, weightAtIndex } from '../lib';
...
describe('Calculate', () => {
  ...
  before(async () => {
    ...
    minW = network.nodes.reduce((p, n) => Math.min(p, n.meta?.numMetric || Number.MAX_SAFE_INTEGER), Number.MAX_SAFE_INTEGER);
    maxW = network.nodes.reduce((p, n) => Math.max(p, n.meta?.numMetric || Number.MIN_SAFE_INTEGER), Number.MIN_SAFE_INTEGER);
  });
  ...
  it ('Should retrieve an edge', async () => {
    ...
    assert(edge);
    edge = getEdge(network, network.nodes[5], { id: 2819382, out: [], in: [], value: -1 });
    assert(!edge);
  });
